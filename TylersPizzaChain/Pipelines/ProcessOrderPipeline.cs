...
namespace TylersPizzaChain.Pipelines
{
    public static class ProcessOrderPipeline
    {
        public static async Task<OrderConfirmation> Execute(ImpureDependencies impure, OrderDetails input)
        {
            ...
            //process payment
            var paymentResponse = await PaymentProcessorClient.ProcessPayment(impure._logger, impure._httpClientFactory, savedPayment.VendorPaymentId, orderTotal);
            ...
            var partiallyAppliedPaymentProcessor = PaymentProcessorClient.ProcessPaymentPartiallyApplied(impure._logger, impure._httpClientFactory);
            var paymentResponsePA = await partiallyAppliedPaymentProcessor(savedPayment.VendorPaymentId, orderTotal);
            if (!paymentResponsePA.IsSuccess) { throw new OrderProcessingException("Payment failed"); }//TODO: parse why payment failed to notify customer
